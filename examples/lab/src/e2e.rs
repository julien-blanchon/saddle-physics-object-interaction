...
use saddle_physics_object_interaction::ObjectInteractionSystems;

use crate::scenarios;

pub struct ObjectInteractionLabE2EPlugin;

impl Plugin for ObjectInteractionLabE2EPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(saddle_bevy_e2e::E2EPlugin);
        app.configure_sets(
            Update,
            saddle_bevy_e2e::E2ESet.before(ObjectInteractionSystems::ReadCommands),
        );
        ...
        if let Some(name) = scenario_name {
            if let Some(mut scenario) = scenarios::scenario_by_name(&name) {
                ...
                error!(
                    "[object_interaction_lab:e2e] Unknown scenario '{name}'. Available: {:?}",
